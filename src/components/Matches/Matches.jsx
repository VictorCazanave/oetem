...
class Matches extends Component {
			...
			this.cancelablePromise.promise
				.then(responses => {
					...
					for (const response of responses) {
						matches = matches.concat(this.getMatches(response.area, selected.temperature, selected.skys));
					}
	...
	getMatches(area, selectedTemperature, selectedSkys) {
		return area.locations.filter(location => this.match(location.weather, selectedTemperature, selectedSkys)).map(matchedLocation => {
	...
	match(weather, selectedTemperature, selectedSkys) {
		return (
			weather.minTemperature.value >= selectedTemperature.min &&
		...
		return (
			<section className="matches-page">
				<header className="matches-page__header">
					...
					{this.valid && this.fetching && <Spinner />}
					{this.valid &&
						!this.fetching &&
					...
					{this.valid &&
						!this.fetching &&
						this.state.matches.length === 1 && <p className="matches-page__header__subtitle">Here is the perfect place for you:</p>}
					{this.valid &&
						!this.fetching &&
						this.state.matches.length > 1 && (
							<p className="matches-page__header__subtitle">{this.state.matches.length}&nbsp;places match your criteria:</p>
						)}
				</header>
				...
				<ul className="matches-page__list">
					{this.valid &&
						...
						this.state.matches.map(match => (
							<li className="matches-page__list__item" key={`${match.area.id}${match.name}`}>
								<Match match={match} />
							</li>
						))}
				</ul>
				...
				<footer className="matches-page__footer">
					<Link to="/" className="matches-page__footer__button-home">
						Home
					</Link>
					<Link to={this.props.againPath} onClick={this.props.onClickAgain} className="matches-page__footer__button-again">
						Try again
					</Link>
				</footer>
			</section>
...
Matches.propTypes = {
	...
	againPath: PropTypes.string,
	onClickAgain: PropTypes.func
};
